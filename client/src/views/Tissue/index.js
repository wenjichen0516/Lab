...
function PageTissue() {
    ...
    const url = `${appConfig.baseURL}/api/tissue`;
    ...
    const save = () => {
        ...
        if (msg.length > 0) {
            ...
            if (editFlag) {
                ...
                axios.put(editURL, { tissue: tissue })
                    .then((resp) => {
                ...
                axios.post(editURL, { tissue: tissue })
                    .then((resp) => {
    ...
    const TissueTable = (props) => {
        const filteredItems = props.tissueList.filter(item =>
            (
                item.tissue && item.tissue.toLowerCase().includes(props.searchFilter.toLowerCase())
            )
        )

        return (
            props.tissueList.length > 0 &&
            <DataTable
                className="table table-bordered table-primary no-footer"
                noHeader={true}
                columns={props.columns}
                data={props.searchFilter.trim().length > 0 ? filteredItems : props.tissueList}
                highlightOnHover
                pagination={true}
                wrap={true}
        ...
        <div className="mainpanel" style={{ marginLeft: '5px' }}>
            <div className="row">
                <div className="pageheader">
                    <h1 className="QC">Tissue Table</h1>
                </div>
            </div>
                                            ...
                                            <div className="form-horizontal">
                                                <div className="form-group">
                                                    ...
                                                    <div className="col-lg-8 col-md-8 col-sm-8">
                                                        <input type="text" className="form-control" onKeyUp={(e) => { setTissue(e.target.value); }} placeholder="Tissue" defaultValue={modal.tissue} />
                                                    </div>
                                                </div>
...
export default React.memo(PageTissue)